...
type typ =
  ...
  | TBool (* Type bool *)
  | TChar (* Type char *)
  | TArray of typ * int32 option (* Array type *)
...
type inctype = Pre | Post [@@deriving show, ord, eq]

type assigntype = Eq | Plus | Minus | Times | Div | Mod [@@deriving show, ord, eq]

type 'a expr = ('a expr_node, 'a) annotated_node

and 'a expr_node =
  | LV of 'a lvalue (* x or a[e] *)
  | Assign of assigntype * 'a lvalue * 'a expr (* x=e or a[e]=e *)
  | Increment of inctype * 'a lvalue * int32
  | ILiteral of int32 (* Integer literal *)
...
and 'a stmtordec_node =
  | LocalDecl of vdecl * 'a expr option (* Local variable declaration *)
